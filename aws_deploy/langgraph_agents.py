...
class DatabaseManager:
    ...
    def get_pg_connection(self):
        ...
        if not self.pg_conn:
            try:
                # AWS 환경에서는 환경 변수에서 DB 정보 가져오기
                if os.getenv('AWS_ENVIRONMENT') == 'true':
                    self.pg_conn = psycopg2.connect(
                        dbname=os.getenv('DB_NAME', 'youth_policy'),
    ...
    def load_vector_db(self):
        ...
        try:
            ...
            # AWS 환경에서는 /tmp에서 로드, 로컬에서는 data/에서 로드  
            if os.getenv('AWS_ENVIRONMENT') == 'true':
                db_path = "/tmp/vector_db_openai_large_combined"
            else:
            ...
            if os.path.exists(db_path):
                ...
                logging.error(f"Vector DB 파일을 찾을 수 없습니다: {db_path}")
                raise FileNotFoundError(f"Vector DB 파일을 찾을 수 없습니다: {db_path}")
    ...
    try:
        ...
        for i, doc in enumerate(retrieved_docs):
            ...
            policy_info = {
                ...
                "정책지원내용": doc.page_content[:500] if doc.page_content else "N/A",

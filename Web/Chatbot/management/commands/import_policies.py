...
from Chatbot.models import YouthPolicy
from Chatbot.graph_models import PolicyNode

class Command(BaseCommand):
    help = '청년정책 CSV를 PostgreSQL과 Neo4j에 저장'

    def add_arguments(self, parser):
        parser.add_argument('csv_path', type=str)

    def handle(self, *args, **options):
        df = pd.read_csv(options['csv_path'], dtype=str).fillna('')
        ...
        self.stdout.write(self.style.SUCCESS('CSV 데이터 저장 완료'))